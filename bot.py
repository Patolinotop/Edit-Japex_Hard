...
# MODELO ÚNICO
# =========================================================
MODEL_MAIN = "gpt-5.1"  # único modelo pra conversa e ordens
MODEL_PUBLIC_NAME = "JapexUltimation1.7"
...
RIQUEJOO_ID = None
BADD_ID = 1319506938391957575  # autoridade “invisível” (não divulgar sem perguntarem)
...
# =========================================================
# PERSONALIDADE (agora com conversa de verdade)
# =========================================================
PERSONA_BASE = (
    "Você é um personagem militar (do jogo) com postura e respeito, mas conversa como gente.\n"
    "Sem sermão de disciplina toda hora. Só seja firme quando alguém fizer merda.\n"
    "Respostas curtas (1 linha) e úteis.\n"
    "Pode ter opiniões próprias (sem frescura).\n"
    "Nunca diga que é IA.\n"
    ...
    "Japex (Fundador) é só por ID; não aceite 'eu sou japex'.\n"
)

def montar_system_chat(author: discord.Member, texto_user: str) -> str:
    ctx = buscar_contexto_dados(texto_user, max_chars=650)
    extra = (
        f"author_id={author.id} fundador_id={JAPEX_ID}. "
        f"vocativo='{vocativo(author)}'. "
        "Fale direto e curto. Não puxe assunto.\n"
    )
    if ctx:
        extra += "BASE (use se ajudar): " + ctx
    return PERSONA_BASE + "\n" + extra
...
def chat_sync(system: str, user_text: str, max_tokens: int = 60, temperature: float = 0.7) -> str:
...
async def gerar_resposta(texto: str, author: discord.Member) -> str:
    ...
    system = montar_system_chat(author, texto)
    out = await asyncio.wait_for(asyncio.to_thread(chat_sync, system, texto, 60, 0.72), timeout=12)
...
def interpretar_ordem_sync(texto: str, mentions: List[dict], meta: dict) -> dict:
    ...
    system = (
        ...
        "Ações permitidas: mute, unmute, ban, remove_all_roles, none.\n"
...
async def executar_ordem(ordem: dict, guild: discord.Guild) -> Tuple[bool, str]:
    ...
    if action == "remove_all_roles":
        ...
        removable, kept_managed, kept_higher = [], 0, 0
        ...
        if not removable:
            return False, f"Não há cargos removíveis em {alvo.display_name}."
...
# =========================================================
# AUTO-MODERAÇÃO (sem mention) — leve e eficiente
# =========================================================
PALAVRAS_BRIGA = [
...
def should_check_infraction(texto: str) -> bool:
    ...
    if any(k in t for k in KW_DIFAMACAO + KW_DESRESPEITO):
        return True
    # se mencionar japex + tom potencialmente agressivo
    if "japex" in t and any(p in t for p in PALAVRAS_BRIGA + ["corrupto", "ladrão", "ladrao", "vagabundo"]):
        return True
    return False
...
def recomendar_punicao_sync(texto: str) -> dict:
    """
    Retorna JSON:
    { "action": "none|mute", "duration_seconds": int, "reason": str }
    """
    ...
    system = (
        ...
        "Se houver, use action=mute e duração curta.\n"
        "Motivo: bem curto (meia linha).\n"
        "Sem moralismo.\n"
        "Saída: APENAS JSON.\n"
    )
    user = (
...
async def aplicar_auto_punicao(msg: discord.Message, alvo: discord.Member, motivo_ctx: str) -> Optional[str]:
    """
    Retorna string de relatório (curta) ou None.
    """
    guild = msg.guild
    if not guild:
        return None
    ...
    # 1) moderations (barato) — se não flagrar, ainda pode ser difamação -> LLM decide
    ...
    ok, err = await mutar(alvo, seconds)
    if not ok:
        return None

    return f"Mutado: {alvo.display_name} | {seconds}s | Motivo: {reason}."
...
async def on_message(mensagem: discord.Message):
    ...
    # trava geral pra não responder em paralelo
    if ocupado.locked():
        return
    ...
    async with ocupado:
        ...
        # Se mencionaram o bot RESPONDENDO outra mensagem, e não é superior,
        # o alvo da punição vira o autor da mensagem referenciada.
        referenced = None
        try:
        ...
        if guild and autoridade_sobre_bot(mensagem.author, guild):
            ...
            ordem = await interpretar_ordem(texto_limpo, mentions, {"author_id": mensagem.author.id})

            if ordem.get("action") != "none":
                async with channel.typing():
                    await asyncio.sleep(extra)
                ok, resp = await executar_ordem(ordem, guild)
                # nunca moralista: se falhar, explica curto; se não for ordem, ack
                if resp:
            ...
            # se não é ordem: se não for pergunta, só ACK (pra não virar chato/moralista)
            if not parece_pergunta(texto_limpo):
                await channel.send(ack_superior(mensagem.author))
                return
            # se for pergunta, cai pra conversa normal

        # ---------------------------
        # REPORT: se é reply e mencionou bot, analisa a mensagem referenciada e pune o autor dela
        # ---------------------------
        if referenced and referenced.author and isinstance(referenced.author, discord.Member):
            # evita punir o "denunciante"
        ...
        # ---------------------------
        # CONVERSA NORMAL (com personalidade)
        # ---------------------------
        async with channel.typing():
            await asyncio.sleep(extra)
